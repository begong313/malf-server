...
export class ChatRoomModel {
    ...
    private getLoadMyChatRoomsQuery(): string {
        const query: string = `select post.post_id, post.picture as picture, post.title as title, (select count(*) from post_participation where post_id = post.post_id) as personnel from post_participation join post on post_participation.post_id = post.post_id where post_participation.user_uniq_id = ? and post_status != 3`;
        return query;
    }
}
